...
const { unauthorized, forbidden, serverError } = require("../utils/responses.js");
...
exports.protect = async (req, res, next) => {
    ...
    try {
        ...
        if (!currentUser) {
            logger.warn(`Unauthorized: Token belongs to a deleted or inactive user (ID: ${data?.id}).`);
        ...
        if (changedPassword) {
            logger.info(`User (ID: ${data?.id}) recently changed password. Token invalidated.`);
            return unauthorized(res, "Your password was recently changed. Please log in again.");
        };
        ...
        return serverError(res, "Something went wrong while verifying your session. Please try again later.", error);
    } finally {
