...
exports.register = async (req, res) => {
    ...
    try {
        ...
        if (!newUser) {
            logger.error(`BadRequest: Failed to create user --> ${JSON.stringify({ name, email, password })}`);
        ...
        if (!token) {
            logger.error(`Failed to create JWT token for user: ${newUser.id}. Deleting user.`);
        ...
        // Success Response
        return created(res, { token, user: newUser }, "User registered successfully");
    } catch (error) {
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        // Success Response
        return success(res, { token, user: User.sanitize(user) }, "Logged in successfully");
    } catch (error) {
...
exports.forgot_password = async (req, res) => {
    ...
    try {
        ...
        if (!resetToken) {
            logger.error(`ServerError: Failed to create password reset token for user ID: ${user.id}`);
        ...
        if (!updatedUser) {
            logger.error(`ServerError: Failed to update user after token generation for email: "${email}"`);
            return serverError(res, "Failed to update user record. Please try again later.");
        }
        ...
        try {
            ...
            // Error Response
            logger.error("Error while sending reset email: ", JSON.stringify({ message: mailError?.message || "", stack: mailError?.stack }));
            return serverError(res, "Error sending reset email. Please try again later.", mailError);
        }
    } catch (error) {
        ...
        return serverError(res, "Internal server error occured while forgot password.", error);
    } finally {
        logRequestDetails(req);
    }
};
...
exports.reset_password = async (req, res) => {
    ...
    try {
        ...
        // Success response
        return success(res, { token: authToken, user }, "Password reset successful");
    } catch (error) {
...
exports.get_me = async (req, res) => {
    ...
    try {
        ...
        logger.error(`Error occured while retrieving user data: `, JSON.stringify({ message: error?.message || "", stack: error?.stack }));
...
exports.update_me = async (req, res) => {
    ...
    try {
        ...
        if (email) {
            const checkEmail = await User.findData(`id <> ${req.user.id} AND email ILIKE '${email}'`);
            if (checkEmail) {
                logger.warn(`Conflict: Email already in use -> ${email}`);
                return conflict(res, `The email "${email}" is already registered with another account.`);
            }
        };
        ...
        if (!updateData) {
            ...
            return serverError(res, "Something went wrong while updating your profile.\nPlease try again later.");
        };
        ...
        return serverError(res, "Internal server error occurred while updating your profile.", error);
    } finally {
        logRequestDetails(req);
    }
};
...
exports.update_password = async (req, res) => {
    ...
    try {
        ...
        if (!updated) {
            ...
            return serverError(res, "Something went wrong while updating your password.\nPlease try again later.");
        };
        ...
        // Success Response
        return success(res, { token, user }, "Your password has been updated successfully");
    } catch (error) {
...
exports.user_list = async (req, res) => {
    ...
    try {
        const users = await User.findAllData({ role: "user" }, "id, name, email, role, active");
        return success(res, users, "Users retrieved successfully");
    } catch (error) {
        console.log("Error user list:", error?.message || "");
        logger.error("Error during user list: ", JSON.stringify({ message: error?.message || "", stack: error?.stack }));
...
exports.user_list_pagination = async (req, res) => {
    ...
    try {
        ...
        // Sucess message
        return paginated(res, data, totalUsers, page, limit, "Users retrieved successfully");
    } catch (error) {
...
exports.get_user = async (req, res) => {
    ...
    try {
        ...
        return serverError(res, "Internal server error while retrieving user.", error);
    } finally {
        logRequestDetails(req);
    }
};
