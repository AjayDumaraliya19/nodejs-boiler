...
exports.insertQuery = async (client, tbl, insertObj, returnData = "*") => {
  ...
  try {
    ...
    console.log(
      `Error occured insertQuery function: ${JSON.stringify({ message: error?.message, stack: error?.stack || "" })}`,
    );
    obj.error = error;
  } finally {
    return obj;
  ...
  try {
    ...
    if (query) {
      ...
      if (typeof query === "object") {
        ...
        whereClause = Object.keys(query)
          .map((key, ind) => `${key} = $${ind + 1}`)
          .join(" AND ");
      }
    ...
    console.log(
      `Error occured getListQuery function: ${JSON.stringify({ message: error?.message, stack: error?.stack || "" })}`,
    );
    obj.error = error;
    return obj;
  } finally {
...
/* -------------------------------------------------------------------------- */
exports.getPageListQuery = async (
  client,
  tbl,
  fields,
  whereClause,
  ...
  try {
    ...
    if (hasSearch) {
      ...
      searchConditions = colum
        .map((col, ind) => `${col} ILIKE $${ind + 1}`)
        .join(" OR ");
    ...
    console.log(
      `Error occured getPageListQuery function: ${JSON.stringify({ message: error?.message, stack: error?.stack || "" })}`,
    );
    obj.error = error;
    return obj;
  } finally {
...
exports.updateQuery = async (
  ...
  try {
    ...
    const whereClause = Object.keys(query)
      .map(
        (key, index) =>
          `${key} = $${index + Object.keys(formattedData).length + 1}`,
      )
      .join(" AND ");
    ...
    console.log(
      `Error occured updateQuery function: ${JSON.stringify({ message: error?.message, stack: error?.stack || "" })}`,
    );
    obj.error = error;
    return obj;
  } finally {
...
exports.deleteQuery = async (client, tbl, condition) => {
  ...
  try {
    ...
    console.log(
      `Error occured deleteQuery: ${JSON.stringify({ message: error?.message, stack: error?.stack || "" })}`,
    );
    obj.error = error;
    return obj;
  } finally {
