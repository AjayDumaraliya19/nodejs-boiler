...
const {
    ...
    getListQuery,
    updateQuery,
    getPageListQuery } = require("./common.model.js");
...
class User {
    ...
    static sanitize(userData) {
        ...
        const { password, password_reset_token, password_reset_expires, password_changed_at, ...cleanData } = userData;
        return cleanData;
    }
    ...
    static async findAllData(user, fields) {
        try {
            ...
            const { data: users, error } = await getListQuery(pool, tbl1, user, fields);
            if (error) {
    ...
    static async findDataWithPagination(whereClause, fields, page, limit, search, columns, order) {
        try {
            const { data: result, error } = await getPageListQuery(pool, tbl1, fields, whereClause, page, limit, search, columns, order);
            if (error) {
    ...
    static async findByEmail(userReq) {
        try {
            const { data: users, error } = await getListQuery(pool, tbl1, { email: userReq?.email });
            if (error) {
            ...
            if (userReq?.password) {
                ...
                if (!isPasswordValid) {
                    logger.warn(`Unauthorized: Incorrect password for user "${userReq?.email}"`);
                    return null;
                };
            };
    ...
    static async findByToken({ token, newPassword }) {
        try {
            ...
            const { data: users, error } = await getListQuery(pool, tbl1, whereClause);
            if (error) {
            ...
            if (!updatedUser) {
                logger.error(`Failed to update password for user: ${JSON.stringify(user)}`);
                return null;
            };
    ...
    async save() {
        try {
            const { id, ...rest } = this;
            const { data: updated, error } = await updateQuery(pool, tbl1, { id }, rest);
            if (error) {
